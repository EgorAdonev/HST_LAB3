...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        for (int index_of_matrix = 0; index_of_matrix < count_of_matrices; index_of_matrix++)
        {
            ...
            for (int row = 0; row < rank_of_matrix; row++)
            {
                for (int column = 0; column < rank_of_matrix; column++)
                {
                    matrix[row * rank_of_matrix + column] = 0;
                    for (int rank = 0; rank < rank_of_matrix; rank++)
                    {
                        matrix[row * rank_of_matrix + column] += buffer_matrix[row * rank_of_matrix + rank] * buffer_matrix[rank * rank_of_matrix + column];
                    }
                }
            }
